...
function (coords, z, method = "Moran", nbclass = NULL, zero.policy=NULL, ...)
{
    ...
    class(res) <- c("correlogram", "matrix")
    res
}

"plot.correlogram"<-function (x,type,xlab,ylab,main,...) {
  if (!inherits(x, "correlogram")) stop("Object must be of class 'correlogram'")
